...
SphereMesh::SphereMesh(int rings, int segments) : DX12Mesh()
{
	float pi = Math::PI;
	float radius = 0.5f;
	...
	Vector3 top(0.0f, radius, 0.0f);
	Vector3 bottom(0.0f, -radius, 0.0f);
	...
	{
		pushNormal(Vector3((float)rand() / (float)(RAND_MAX), (float)rand() / (float)(RAND_MAX), (float)rand() / (float)(RAND_MAX)));
	}
}
