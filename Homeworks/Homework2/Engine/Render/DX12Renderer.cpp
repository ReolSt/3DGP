...
namespace Engine
{
    IDXGIFactory4* DX12Renderer::getFactory()
    {
    ...
    void DX12Renderer::initialize(HINSTANCE instance, HWND window)
    {
        m_instance = instance;
        m_window = window;
    ...
    void DX12Renderer::createDevice()
    {
        ...
        for (UINT i = 0; SUCCEEDED(m_factory->EnumAdapters1(i, &hardwareAdapter)); ++i)
        {
            ...
            if (SUCCEEDED(D3D12CreateDevice(hardwareAdapter, D3D_FEATURE_LEVEL_12_0, IID_PPV_ARGS(&m_device)))) break;
        }
    ...
    void DX12Renderer::createDSVHeap()
    {
        ...
        m_device->CreateDescriptorHeap(&descriptorHeapDesc, IID_PPV_ARGS(&m_dsvHeap));
        m_dsvDescriptorSize = m_device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
    }

    void DX12Renderer::createCBVHeap()
    {
        D3D12_DESCRIPTOR_HEAP_DESC descriptorHeapDesc;
        ZeroMemory(&descriptorHeapDesc, sizeof(D3D12_DESCRIPTOR_HEAP_DESC));
    ...
    void DX12Renderer::waitForGPUComplete()
    {
    ...
    void DX12Renderer::render(GameEngine* engine)
    {
        ...
        world->getRegisteredComponentsByTypeId(GetTypeId(DX12MeshComponent), meshes);
        ...
        for (auto& component : meshes)
        {
            ...
            auto meshComponent = dynamic_cast<DX12MeshComponent*>(component);
        ...
        for (auto& component : cameras)
        {
            ...
            for (auto& component : meshes)
            {
                ...
                m_commandList->SetGraphicsRoot32BitConstants(1, 16, meshWorldTransform.toMatrix().transpose().getDXMatrix(), 0);

                m_commandList->SetPipelineState(shader->getPipelineState());

                auto vertexBuffer = mesh->getVertexBuffer();
                auto normalBuffer = mesh->getNormalBuffer();
                auto indexBuffer = mesh->getIndexBuffer();

                auto& vertexBufferView = mesh->getVertexBufferView();
                auto& normalBufferView = mesh->getNormalBufferView();
                auto& indexBufferView = mesh->getIndexBufferView();

                m_commandList->IASetVertexBuffers(0, 1, &vertexBufferView);
                m_commandList->IASetVertexBuffers(1, 1, &normalBufferView);
